...
class MPC(object):
    ...
    def __init__(self, model, dynamics,
        ...
        self.dt = model.dt #Same sampling as model
        self.Nx, self.Nu = model.n, model.m
        self.Nt = N #Horizon
        print("Horizon steps: ", N * self.dt) #Horizon time
        ...
        self.Q = ca.MX(Q) #Need
        self.P = ca.MX(P)
        self.R = ca.MX(R) #Difference SX and MX variables? SX faster for graph opt and MX more for matrix computation.

        #set constraints: None to constraints: Set to inv.
        if xub is None:
        ...
        u0 = ca.MX.sym('u0', self.Nu) #optional. For linearizing. If reference control is not zero. Asymptotically stable needed. Integral action not zero.
        
        param_s = ca.vertcat(x0, x0_ref, u0) #for the solver. 1-d vector needed.

        # Create optimization variables #Needed for excess to variables.
        ...
        # Set initial values
        obj = ca.MX(0) #MX fuction. Cost function will be a symbolic function.
        con_eq = []
        con_ineq = []
        con_ineq_lb = []
        con_ineq_ub = []
        con_eq.append(opt_var['x', 0] - x0) #First step of optimization should be x0. Negative constraints needed since: a - b = 0
        ...
        for t in range(self.Nt):
            ...
            # Dynamics constraint
            x_t_next = self.dynamics(x_t, u_t) #Linear or nonlinear function of dynamics possible. Control the linearization problem by yourself.
            con_eq.append(x_t_next - opt_var['x', t + 1]) #equallity contraints (6b) subject to step.

            # Input constraints
            if uub is not None: #extra evaluations
                con_ineq.append(u_t)
                con_ineq_ub.append(uub)
                con_ineq_lb.append(np.full((self.Nu,), -ca.inf)) #try using as few constraints as possible. Two inequality contraints or 1 equ.
            if ulb is not None:
                con_ineq.append(u_t)
        ...
        # Terminal Cost
        obj += self.terminal_cost(opt_var['x', self.Nt] - x0_ref, self.P) #xt of the last step needed.
        ...
        num_eq_con = ca.vertcat(*con_eq).size1() #just for logging
        num_ineq_con = ca.vertcat(*con_ineq).size1()
        con_eq_lb = np.zeros((num_eq_con,))
        con_eq_ub = np.zeros((num_eq_con,))

        # Set constraints
        con = ca.vertcat(*con_eq, *con_ineq) #star unpacks the structure
        ...
        nlp = dict(x=opt_var, f=obj, g=con, p=param_s) #x=opt_var (u and x, but x constrained by dynamics), just with u would also be ok. Solver creation takes much more time than solving one step.
        options = {
            'ipopt.print_level': 0,
            'print_time': False,
            'verbose': False,
        ...
        self.solver = ca.nlpsol('mpc_solver', 'ipopt', nlp, options) #ipopt is a nice solver.
    ...
    def set_cost_functions(self):
        ...
        Q = ca.MX.sym('Q', self.Nx, self.Nx) #MX means symbolic, so by init of cost function, DM would mean numeric. 
        R = ca.MX.sym('R', self.Nu, self.Nu)
        P = ca.MX.sym('P', self.Nx, self.Nx)

        x = ca.MX.sym('x', self.Nx) #x and u has to be symbolic because they have to stay variable over the optimization
        u = ca.MX.sym('u', self.Nu) #If you do it as numeric, the solver must to be init every opt step -> not possbile, time consuming.
    ...
    def solve_mpc(self, x0, u0=None):
        ...
        self.optvar_init['x', 0] = self.optvar_x0[0] #might some time.
